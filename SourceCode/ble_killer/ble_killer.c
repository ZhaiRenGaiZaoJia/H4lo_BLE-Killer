    ...
    if(input_event->type == InputTypeShort) {
        if(input_event->key == InputKeyLeft) {
            ...
            view_dispatcher_send_custom_event(
                _ctx->view_dispatcher, START_SCAN);
            return true;
        }
    }
    return false;
...
bleScanCtx* ble_init() {
    bleScanCtx* app = malloc(sizeof(bleScanCtx));
    ...
    */
    ...
    variable_item_set_current_value_text(credits, "模块");
...
*/
    ...
    view_set_context(app->weapon_view, app);
